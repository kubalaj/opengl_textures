...
static void room(double x,double y,double z,
   ...
   // Used from ex15 from the cube function
    //  Set specular color to white
   float white[] = {1,1,1,1};
   float Emission[]  = {0.0,0.0,0.01*emission,1.0};
   glMaterialfv(GL_FRONT_AND_BACK,GL_SHININESS,shinyvec);
...
void display()
{
   ...
   //chair
   //base
   cube(0,-.75,0 , 0.5,0.25,0.5 , 0);
   //front right leg
   cube(.4,-1.1,.4, 0.05,1.5,0.05 , 0);
   //front left leg
   cube(-.4,-1.1,.4, 0.05,1.5,0.05 , 0);
   //back left leg
   cube(-.4,-1.1,-.4, 0.05,1.5,0.05 , 0);
   ...
   //Chair Backrest
   //right
   cube(.4,-.5,-.4, 0.05,1.5,0.05 , 0);
   //left
   cube(-.4,-.5,-.4, 0.05,1.5,0.05 , 0);
   //back
   cube(0,0,-.4, .5,.5,0.05 , 0);
   room(0,-.5,0 , 3.0,1.0,3.0 , -45);
...
int main(int argc,char* argv[])
{
   ...
   glutIdleFunc(idle);
   //  Load textures
   texture[0] = LoadTexBMP("metalwall.bmp");
